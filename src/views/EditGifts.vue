...
export default {
  ...
  data() {
    ...
    methods: {
      ...
      getGiftsInfo () {
          $.get(baseUrl + 'promise/get',{token: this.token, id: this.id}, (data) => {
              if (data.status == 200) {
                this.ruleForm = data.bean
                this.ruleForm.promiseStatus = this.ruleForm.promiseStatus ? "1":"0"
                this.ruleForm.promiseType = this.ruleForm.promiseType.toString()
              }
          })
      },
      resetForm(formName) {
