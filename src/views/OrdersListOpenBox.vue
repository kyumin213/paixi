        ...
        <el-table
            ...
            <el-table-column label="操作" width="250">
              <template slot-scope="scope">
                    ...
                    <el-button v-show="(scope.row.addressId != 'PX_6666666666666' && scope.row.addressId  !='PX_5555555555555') && scope.row.status == 2"
                          size="mini"
                          @click="sendOrderIds(scope.row)">发送物流</el-button>

                    <el-button v-show="scope.row.status != 9"
                        size="mini"
...
export default {
  ...
  data() {
    return {
      ...
      categoryName: ["商城订单","悄悄订单","开箱订单","拼图订单","问答订单","AA订单","摇骰订单","直接送订单","免费活动订单"],
      kuaidi: [
        {
          name: "顺丰",
          id: 'shunfeng'
      ...
      if (this.valueDate == null || this.valueDate.length == 0) {
        $.get(
          ...
          data => {
            ...
            this.i++
            setTimeout(()=>{
                exportTb('#exTable',"开箱子订单所有时间" + '.xlsx')                    
            },2000)
          }
        )
      } else {
        $.get(
          ...
          data => {
            ...
            this.i++
            setTimeout(()=>{
                exportTb('#exTable',"开箱子订单 "+ this.valueDate[0] +'至'+this.valueDate[1] + '.xlsx')                            
            },2000)
          }
        )
      }
